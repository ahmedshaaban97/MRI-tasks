...
#       # QApplication.processEvents()
    ...
    def select_Preparetion(self):
        ...
        if (self.ui.Preparetion_Box.currentText()=="T1 Prep (IR)"):
            ...
            Tnull = self.tnull
            self.phantom = self.T1_preparation(self.phantom,row,col,Tnull,self.T1)
        elif (self.ui.Preparetion_Box.currentText()=="T2  Prep"): 
            self.T2_PREP()
            self.phantom = self.createPhantom(row,col)
            self.phantom=self.T2_preparation(self.time,self.T1,row,col,self.phantom)
...
#            self.TE=int(self.ui.TE_Edit.text())
    ...
    def T2_preparation(self,Tw,T1,row,col,phantom):
    ...
    def T1_preparation(self,phantom,row,col,Tnull,T1):
        theta=np.radians(180)
        T=np.log(2)*Tnull
        phantom=self.RF_rotate(theta,self.phantom,row,col)
        phantom=self.recovery(self.phantom,row,col,T,T1)
        return phantom
